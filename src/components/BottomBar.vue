    ...
    <section class="bottom-bar responsive-container">
      <template v-for="nav in navigations" :key="nav.id">
        <span
          ...
          :class="
            routeName === nav.name ? 'text-green-200' : 'text-gray-500 scale-95'
          "
          class="flex flex-col items-center active:scale-95 duration-300"
        >
          <i :class="nav.icon" class="text-2xl"></i>
          <i
            v-if="routeName === nav.name"
            class="text-green-200 text-xxs fa fa-circle"
          ></i>
        </span>
      </template>
    </section>
...
const mainNavigations = ['Home', 'Marketplace', 'Profile'];
