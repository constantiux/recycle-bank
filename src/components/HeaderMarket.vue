  ...
  <HeaderBar>
    <template v-slot:start>
      <section class="flex items-center gap-3">
        <div>
          <h1 class="font-semibold text-xl">Marketplace</h1>
          <p class="text-sm">
            Buy recycled goods and support green communities
          </p>
        </div>
      </section>
    </template>
          ...
          <span
            :class="
              currentCategory === item.id
                ? 'bg-green-600 text-gray-200 font-medium px-3 py-1 rounded-full'
                : ''
            "
            @click="currentCategory = item.id"
            class="active:95 duration-300"
            >{{ item.name }}</span
          >
        </template>
      </section>
